...
final class WebXmlParser {
    ...
    /** 流式累计时检查上限，避免描述符耗尽内存。 */
    private static byte[] readBounded(InputStream input) throws IOException, WebAppDeploymentException {
    ...
    private static void parseRoot(Element root, WebXmlModel model, boolean fragment)
        ...
        for (Element element : children(root)) {
            ...
            if ("context-param".equals(name)) {
                ...
                putUnique(model.contextParameters, key, requiredChildText(element, "param-value"),
                        "context parameter");
                ...
                if (model.denyUncoveredHttpMethods) {
                    throw new WebAppDeploymentException(
                            "duplicate deny-uncovered-http-methods");
                }
            ...
            } else if (!"display-name".equals(name) && !"description".equals(name)
    ...
    private static void parseMimeMapping(Element element, WebXmlModel model)
        ...
        if (previous != null && !previous.equals(mimeType)) {
            throw new WebAppDeploymentException(
                    "conflicting MIME mapping for extension " + extension);
        }
    }
    ...
    private static void parseLocaleEncodingMappings(Element element, WebXmlModel model)
        ...
        for (Element mapping : childElements(element, "locale-encoding-mapping")) {
            ...
            String encoding = canonicalCharacterEncoding(
                    requiredChildText(mapping, "encoding"));
            String previous = model.localeEncodingMappings.put(locale, encoding);
            if (previous != null && !previous.equals(encoding)) {
                throw new WebAppDeploymentException(
                        "conflicting locale encoding mapping for " + locale);
            }
        }
    }
    ...
    private static WebXmlModel.ServletModel parseServlet(Element element)
        ...
        Set<String> supported = set("servlet-name", "servlet-class", "init-param",
                "load-on-startup", "async-supported", "multipart-config", "security-role-ref",
                "description", "display-name");
        rejectUnsupportedChildren(element, supported);
        for (Element parameter : childElements(element, "init-param")) {
            putUnique(model.initParameters, requiredChildText(parameter, "param-name"),
                    requiredChildText(parameter, "param-value"), "Servlet init parameter");
        }
        for (Element reference : childElements(element, "security-role-ref")) {
            rejectUnsupportedChildren(reference,
                    set("description", "role-name", "role-link"));
            putUnique(model.roleReferences,
        ...
        if (multipart != null) {
            rejectUnsupportedChildren(multipart,
                    set("location", "max-file-size", "max-request-size", "file-size-threshold"));
            ...
            model.multipartConfig = new MultipartConfigElement(
    ...
    private static WebXmlModel.FilterModel parseFilter(Element element)
            throws WebAppDeploymentException {
        rejectUnsupportedChildren(element,
                set("filter-name", "filter-class", "init-param", "async-supported", "description", "display-name"));
        ...
        for (Element parameter : childElements(element, "init-param")) {
            putUnique(model.initParameters, requiredChildText(parameter, "param-name"),
                    requiredChildText(parameter, "param-value"), "Filter init parameter");
        }
        String async = optionalChildText(element, "async-supported");
        model.asyncSupportedSpecified = async != null;
    ...
    private static WebXmlModel.FilterMappingModel parseFilterMapping(Element element)
            throws WebAppDeploymentException {
        rejectUnsupportedChildren(element, set("filter-name", "url-pattern", "servlet-name", "dispatcher"));
        ...
        for (Element dispatcher : childElements(element, "dispatcher")) {
            try {
                model.dispatchers.add(DispatcherType.valueOf(
                        requiredText(dispatcher).toUpperCase(Locale.ROOT)));
    ...
    private static WebXmlModel.ErrorPageModel parseErrorPage(Element element)
        ...
        if (model.errorCode != null && model.exceptionType != null) {
            throw new WebAppDeploymentException("error-page cannot contain both error-code and exception-type");
        }
        return model;
    }
    ...
    private static WebXmlModel.SessionModel parseSession(Element element)
        ...
        if (cookie != null) {
            rejectUnsupportedChildren(cookie,
                    set("name", "domain", "path", "comment", "http-only", "secure", "max-age"));
        ...
        for (Element tracking : childElements(element, "tracking-mode")) {
            try {
                model.trackingModes.add(SessionTrackingMode.valueOf(
                        requiredText(tracking).toUpperCase(Locale.ROOT)));
            } catch (IllegalArgumentException e) {
    ...
    private static WebXmlModel.LoginModel parseLogin(Element element)
            throws WebAppDeploymentException {
        rejectUnsupportedChildren(element,
                set("auth-method", "realm-name", "form-login-config"));
        String method = requiredChildText(element, "auth-method").toUpperCase(Locale.ROOT);
        if (!"BASIC".equals(method) && !"FORM".equals(method)
        ...
        model.authenticationMethod = "CLIENT-CERT".equals(method)
        ...
        if ("FORM".equals(method)) {
            if (form == null) {
                throw new WebAppDeploymentException(
                        "FORM authentication requires form-login-config");
            }
            rejectUnsupportedChildren(form,
                    set("form-login-page", "form-error-page"));
            model.formLoginPage = requireApplicationPath(
                    requiredChildText(form, "form-login-page"), "FORM login page");
            model.formErrorPage = requireApplicationPath(
                    requiredChildText(form, "form-error-page"), "FORM error page");
        }
        return model;
    }
    ...
    private static java.util.List<WebXmlModel.SecurityConstraintModel> parseSecurityConstraint(
        ...
        rejectUnsupportedChildren(element,
                set("display-name", "web-resource-collection", "auth-constraint",
                        "user-data-constraint"));
        java.util.List<Element> resources = childElements(element, "web-resource-collection");
        if (resources.isEmpty()) {
            throw new WebAppDeploymentException(
                    "security-constraint requires a web-resource-collection");
        }
        ...
        if (userData != null) {
            ...
            try {
                guarantee = ServletSecurityPolicy.TransportGuarantee.valueOf(
                        requiredChildText(userData, "transport-guarantee")
        ...
        for (Element resource : resources) {
            rejectUnsupportedChildren(resource,
                    set("web-resource-name", "description", "url-pattern",
                            "http-method", "http-method-omission"));
            requiredChildText(resource, "web-resource-name");
            WebXmlModel.SecurityConstraintModel constraint =
                    new WebXmlModel.SecurityConstraintModel();
            ...
            if (constraint.patterns.isEmpty()) {
                throw new WebAppDeploymentException(
                        "web-resource-collection requires a url-pattern");
            }
    ...
    private static String parseCharacterEncoding(Element element)
            throws WebAppDeploymentException {
        return canonicalCharacterEncoding(requiredText(element));
    }

    private static String canonicalCharacterEncoding(String value)
            throws WebAppDeploymentException {
        try {
    ...
    static void validateReferences(WebXmlModel model) throws WebAppDeploymentException {
        for (WebXmlModel.ServletMappingModel mapping : model.servletMappings) {
            if (!model.servlets.containsKey(mapping.servletName)) {
                throw new WebAppDeploymentException("undefined Servlet mapping target: " + mapping.servletName);
            }
        }
        for (WebXmlModel.FilterMappingModel mapping : model.filterMappings) {
            if (!model.filters.containsKey(mapping.filterName)) {
                throw new WebAppDeploymentException("undefined Filter mapping target: " + mapping.filterName);
            }
    ...
    private static Element optionalChild(Element parent, String name) throws WebAppDeploymentException {
        Element result = null;
        for (Element child : children(parent)) {
            if (name.equals(localName(child))) {
                if (result != null) {
    ...
    private static String requiredChildText(Element parent, String name) throws WebAppDeploymentException {
    ...
    private static String optionalChildText(Element parent, String name) throws WebAppDeploymentException {
    ...
    private static <T> void putUnique(java.util.Map<String, T> values, String name, T value, String type)
            throws WebAppDeploymentException {
