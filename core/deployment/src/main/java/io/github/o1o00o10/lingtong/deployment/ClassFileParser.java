...
final class ClassFileParser {
  ...
  private static void readMembers(
      DataInputStream input, ConstantPool constants, Set<String> annotations, String source)
      throws IOException, WebAppDeploymentException {
    int count = input.readUnsignedShort();
  ...
  private static void readAttributes(
    ...
    for (int i = 0; i < count; i++) {
      ...
      if ("RuntimeVisibleAnnotations".equals(name) || "RuntimeInvisibleAnnotations".equals(name)) {
        ...
        readAnnotations(
            new DataInputStream(new ByteArrayInputStream(attribute)),
            constants,
            annotations,
            source);
      } else {
  ...
  private static WebAppDeploymentException invalid(String source, String message, Throwable cause) {
