...
public final class LingTongWebSocketInitializer implements ServletContainerInitializer {
    ...
    public void onStartup(Set<Class<?>> classes, ServletContext context) throws ServletException {
        ...
        Set<Class<?>> candidates = classes == null
                ? Collections.<Class<?>>emptySet() : classes;
        ...
        try {
            ...
            Set<ServerEndpointConfig> selectedProgrammatic =
                    new LinkedHashSet<ServerEndpointConfig>();
            for (Class<? extends ServerApplicationConfig> type : configurations) {
                ServerApplicationConfig configuration = type.newInstance();
                Set<Class<?>> selected = configuration.getAnnotatedEndpointClasses(
                        Collections.unmodifiableSet(annotated));
                if (selected != null) selectedAnnotated.addAll(selected);
                Set<ServerEndpointConfig> configured = configuration.getEndpointConfigs(
                        Collections.unmodifiableSet(endpoints));
