...
public final class WebSocketEndpointRegistry {
  ...
  public HttpResponse handshake(HttpRequest request, Executor executor, int maxMessageBytes)
      throws Exception {
    ...
    for (Route route : routes) {
      ...
      return request.isExtendedConnect()
          ? WebSocketHandshake.acceptExtendedConnect(request, handler, executor, maxMessageBytes)
      ...
      for (String segment : segments) {
        ...
        if (segment.startsWith("{")) {
          ...
          if (!parameters.add(parameter)) {
            throw new IllegalArgumentException("duplicate WebSocket path parameter: " + parameter);
          }
          normalized.append("{}");
        } else {
