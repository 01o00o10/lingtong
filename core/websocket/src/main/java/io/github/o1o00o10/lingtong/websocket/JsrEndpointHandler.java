...
final class JsrEndpointHandler implements WebSocketHandler {
    ...
    /** 二进制片段集合。 */
    private final java.io.ByteArrayOutputStream binaryFragments =
    ...
    @Override public List<String> subprotocols() {
        return subprotocol.isEmpty()
                ? Collections.<String>emptyList()
                : Collections.singletonList(subprotocol);
    }

    @Override public boolean perMessageDeflate() { return compressed; }
    ...
    @Override public boolean receivesTextFragments() { return !textStreaming; }

    @Override
    public void onTextFragment(
            WebSocketSession ignored, String fragment, boolean last) throws Exception {
        JsrSession current = requireSession();
    ...
    @Override public boolean receivesBinaryFragments() { return !binaryStreaming; }

    @Override
    public void onBinaryFragment(
            WebSocketSession ignored, ByteBuffer fragment, boolean last) throws Exception {
        JsrSession current = requireSession();
        if (!binaryMessageActive) {
            binaryMessageActive = true;
            binaryPartialDelivery = annotatedBinaryPartial
                    || current.hasBinaryPartialHandler();
        }
        if (binaryPartialDelivery) {
            current.dispatchBinaryPartial(fragment, last);
            if (annotatedBinaryPartial) {
                sendResult(invokeAnnotated(OnMessage.class, fragment.asReadOnlyBuffer(),
                        null, null, last));
            }
        } else {
    ...
    @Override public boolean streamsTextMessages() { return textStreaming; }
    ...
    @Override public boolean streamsBinaryMessages() { return binaryStreaming; }
    ...
    public void onPong(WebSocketSession ignored, ByteBuffer payload) throws Exception {
        ...
        invokeAnnotated(OnMessage.class, new PongMessage() {
            @Override public ByteBuffer getApplicationData() { return copy.asReadOnlyBuffer(); }
        }, null, null);
    }
    ...
    private Object invokeAnnotated(
        ...
        for (Method method : endpoint.getClass().getMethods()) {
            ...
            try {
                return method.invoke(endpoint, arguments(
                        method, methodMessage, closeReason, failure, last));
    ...
    private Object decodedMessage(Method method, Object message) throws Exception {
        ...
        for (Decoder decoder : requireSession().decoders()) {
            try {
                Object decoded = null;
                if (message instanceof String && decoder instanceof Decoder.Text
                    ...
                    decoded = ((Decoder.TextStream) decoder).decode(
                            new StringReader((String) message));
                } else if (message instanceof ByteBuffer && decoder instanceof Decoder.Binary
                        && ((Decoder.Binary) decoder).willDecode(
                                ((ByteBuffer) message).asReadOnlyBuffer())) {
                    decoded = ((Decoder.Binary) decoder).decode(
                            ((ByteBuffer) message).asReadOnlyBuffer());
                } else if (message instanceof ByteBuffer
                        && decoder instanceof Decoder.BinaryStream) {
                    ByteBuffer source = ((ByteBuffer) message).asReadOnlyBuffer();
                    byte[] bytes = new byte[source.remaining()];
                    source.get(bytes);
                    decoded = ((Decoder.BinaryStream) decoder).decode(
    ...
    private Class<?> messageParameter(Method method) {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            if (Session.class.isAssignableFrom(type)
                    || EndpointConfig.class.isAssignableFrom(type)) continue;
    ...
    private Object[] arguments(
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            else throw new IllegalArgumentException("unsupported endpoint parameter " + types[i].getName());
        }
        return values;
    }
    ...
    private static Object convert(String value, Class<?> type) {
        ...
        if (type == char.class || type == Character.class) {
            if (value == null || value.length() != 1) throw new IllegalArgumentException("invalid char path parameter");
