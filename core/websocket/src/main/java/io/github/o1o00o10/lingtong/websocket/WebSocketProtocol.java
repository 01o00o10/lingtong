...
final class WebSocketProtocol implements ConnectionUpgrade, WebSocketSession {
    ...
    WebSocketProtocol(
        ...
        this(handler, executor, maxMessageBytes, negotiatedSubprotocol,
                perMessageDeflate, true, false);
    }
    ...
    static WebSocketProtocol client(
            ...
            String negotiatedSubprotocol,
            boolean perMessageDeflate) {
        return new WebSocketProtocol(handler, executor, maxMessageBytes, negotiatedSubprotocol,
                perMessageDeflate, false, true);
    }
    ...
    private int decodeFrame(byte[] bytes, int offset) throws IOException {
        ...
        if ((first & 0x30) != 0 || compressed && (!perMessageDeflate
                || control || opcode == OP_CONTINUATION)) {
        ...
        if (masked != expectMaskedInput) {
            return failProtocol(expectMaskedInput
                    ? "client frame is not masked" : "server frame is masked", remaining);
        }
        ...
        if (bytes.length - cursor < length) {
            ...
            networkStreamFrame = new NetworkStreamFrame(
                    opcode, fin, mask, length, dispatchInput);
    ...
    private boolean canStreamNetworkFrame(int opcode) {
        ...
        return streams(opcode) && fragmented == null && streamingInput == null
                && partialOpcode == -1;
    }
    ...
    private void beginPartial(int opcode, boolean compressed) {
        ...
        partialTextDecoder = opcode == OP_TEXT
    ...
    private byte[] decodePartialPayload(byte[] payload, boolean last) throws IOException {
        ...
        ByteArrayOutputStream output = new ByteArrayOutputStream(
                Math.min(Math.max(payload.length, 32), maxMessageBytes));
        if (!inflatePartial(payload, output)) return null;
        if (last && !inflatePartial(
                new byte[] {0, 0, (byte) 0xff, (byte) 0xff}, output)) return null;
        return output.toByteArray();
    }

    private boolean inflatePartial(byte[] payload, ByteArrayOutputStream output)
            throws IOException {
    ...
    private void dispatchStreaming(int opcode, MessageInputStream input) {
        if (opcode == OP_TEXT) {
            final Reader reader = new InputStreamReader(input, StandardCharsets.UTF_8.newDecoder()
                    .onMalformedInput(CodingErrorAction.REPORT)
                    .onUnmappableCharacter(CodingErrorAction.REPORT));
            callback(() -> {
                try {
    ...
    public void sendText(String message) throws IOException {
        ...
        if (payload.length > maxMessageBytes) throw new IllegalArgumentException("message exceeds configured limit");
    ...
    public void sendBinary(ByteBuffer message) throws IOException {
        ...
        if (copy.remaining() > maxMessageBytes) throw new IllegalArgumentException("message exceeds configured limit");
    ...
    public void sendPing(ByteBuffer payload) throws IOException {
        ...
        if (copy.remaining() > 125) throw new IllegalArgumentException("ping payload exceeds 125 bytes");
    ...
    public void sendPong(ByteBuffer payload) throws IOException {
        ByteBuffer copy = payload == null ? ByteBuffer.allocate(0) : payload.slice();
        if (copy.remaining() > 125) throw new IllegalArgumentException("pong payload exceeds 125 bytes");
        byte[] bytes = new byte[copy.remaining()];
        copy.get(bytes);
    ...
    public void onTimer(long nowNanos) {
        ...
        if (!open || timeout <= 0L
                || (nowNanos - lastActivityNanos) / 1_000_000L < timeout) return;
        try {
    ...
    @Override
    public CompletionStage<Void> withSendTimeout(
    ...
    public synchronized void close(int statusCode, String reason) throws IOException {
        if (closeSent) return;
        if (!validCloseCode(statusCode)) throw new IllegalArgumentException("invalid close status code");
        byte[] reasonBytes = (reason == null ? "" : reason).getBytes(StandardCharsets.UTF_8);
        if (reasonBytes.length > 123) throw new IllegalArgumentException("close reason exceeds 123 bytes");
    ...
    private synchronized CompletionStage<Void> writeFragmentAsync(
        ...
        if (!first && outboundFragmentOpcode != opcode) {
            return failed(new IllegalStateException(
                    "a different fragmented WebSocket message is in progress"));
        }
        ...
        try {
            ...
            CompletionStage<Void> result = writeFrameAsync(
                    frameOpcode, encoded, first && compressed, last);
    ...
    private synchronized CompletionStage<Void> writeFrameAsync(
        ...
        CompletionStage<Void> result = current.writeAsync(
                encodedFrame(opcode, payload, compressed, fin));
        result.whenComplete((ignored, failure) -> {
    ...
    private ByteBuffer encodedFrame(
        ...
        int header = (payload.length <= 125 ? 2 : payload.length <= 0xffff ? 4 : 10)
                + maskBytes;
    ...
    private byte[] deflateFragment(byte[] payload, boolean last) throws IOException {
        ...
        while (!outboundDeflater.needsInput()) {
            int count = outboundDeflater.deflate(
                    buffer, 0, buffer.length, Deflater.SYNC_FLUSH);
            if (count == 0) break;
    ...
    private byte[] inflate(byte[] payload) throws IOException {
        ...
        ByteArrayOutputStream result = new ByteArrayOutputStream(
                Math.min(Math.max(256, payload.length * 2), maxMessageBytes));
        byte[] buffer = new byte[8192];
        try {
            while (!inflater.needsInput()) {
