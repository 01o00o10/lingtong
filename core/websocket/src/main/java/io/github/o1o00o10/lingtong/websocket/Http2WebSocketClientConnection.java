...
final class Http2WebSocketClientConnection implements UpgradeChannel {
  ...
  private Http2WebSocketClientConnection(Handshake handshake, WebSocketProtocol protocol) {
    ...
    this.writer =
        new ThreadPoolExecutor(
            ...
            new ThreadPoolExecutor.AbortPolicy());
    this.timer = new ScheduledThreadPoolExecutor(1, daemonFactory("lingtong-ws-h2-timer-"));
  ...
  public CompletionStage<Void> writeAsync(ByteBuffer data) {
    ...
    if (data == null) {
      completion.completeExceptionally(
          new IllegalArgumentException("HTTP/2 WebSocket output must not be null"));
      return completion;
    }
    if (!isOpen()) {
      completion.completeExceptionally(new IOException("HTTP/2 WebSocket connection is closed"));
      return completion;
    }
    ...
    try {
      ...
      completion.completeExceptionally(
          new IOException("HTTP/2 WebSocket write queue is full", rejected));
    }
    return completion;
  }
  ...
  private void startReader() {
    daemonFactory("lingtong-ws-h2-read-")
        .newThread(
            () -> {
              try {
  ...
  private void handle(Http2Frame frame) throws IOException, Http2Exception {
    switch (frame.type()) {
      ...
      case Http2Frame.PUSH_PROMISE:
        throw new Http2Exception(1, 0, "unexpected HTTP/2 frame after WebSocket CONNECT");
      default:
    }
  }

  private void handleSettings(Http2Frame frame) throws IOException, Http2Exception {
    if (frame.streamId() != 0 || (frame.flags() & ACK) != 0 && frame.payload().length != 0) {
    ...
    synchronized (flowLock) {
      if (update.initialWindowSet) {
        long adjusted = (long) streamSendWindow + update.peerInitialWindow - peerInitialWindow;
      ...
      if (update.connectEnabledSet) {
        if (peerConnectEnabled && !update.connectEnabled) {
          throw new Http2Exception(1, 0, "ENABLE_CONNECT_PROTOCOL must not revert to zero");
        }
  ...
  private void handleWindowUpdate(Http2Frame frame) throws Http2Exception {
    ...
    if (increment == 0)
      throw new Http2Exception(1, frame.streamId(), "zero HTTP/2 window increment");
    synchronized (flowLock) {
      if (frame.streamId() == 0) {
  ...
  private void handleData(Http2Frame frame) throws IOException, Http2Exception {
    ...
    if (wire.length > 0) {
      sendFrame(new Http2Frame(Http2Frame.WINDOW_UPDATE, 0, 0, intPayload(wire.length)));
      sendFrame(new Http2Frame(Http2Frame.WINDOW_UPDATE, 0, STREAM_ID, intPayload(wire.length)));
    }
    ...
    while (offset < bytes.length) {
      ...
      synchronized (flowLock) {
        ...
        length =
            Math.min(
                bytes.length - offset,
                Math.min(peerMaxFrameSize, Math.min(connectionSendWindow, streamSendWindow)));
  ...
  private static Handshake openHandshake(ClientEndpointConfig config, URI uri)
    ...
    Socket socket = WebSocketClientConnection.openSocket(config, uri, host, port, timeout, true);
    try {
      ...
      if (first.type() != Http2Frame.SETTINGS
          || first.streamId() != 0
          || (first.flags() & ACK) != 0) {
        throw new DeploymentException("HTTP/2 server did not start with a SETTINGS frame");
      }
      Settings settings = parseSettings(first.payload());
      if (!settings.connectEnabled) {
        throw new DeploymentException("HTTP/2 server did not enable extended CONNECT");
      }
      writeFrame(output, new Http2Frame(Http2Frame.SETTINGS, ACK, 0, new byte[0]));
      ...
      writeHeaderBlock(output, encoder.encode(hpackHeaders(uri, headers)), settings.maxFrameSize);
      ...
      return new Handshake(
          ...
          input,
          output,
          settings.connectionWindow,
          settings.streamWindow,
          settings.peerInitialWindow,
          settings.maxFrameSize,
          negotiated.subprotocol,
          negotiated.compressed);
  ...
  private static void writePreface(OutputStream output) throws IOException {
    ...
    writeFrame(output, new Http2Frame(Http2Frame.SETTINGS, 0, 0, settings.array()));
  }

  private static void writeHeaderBlock(OutputStream output, byte[] block, int maxFrameSize)
      throws IOException {
    int offset = 0;
    boolean first = true;
    do {
  ...
  private static Response readResponse(
    ...
    while (true) {
      ...
      if (continuation
          && (frame.type() != Http2Frame.CONTINUATION || frame.streamId() != STREAM_ID)) {
        throw new Http2Exception(1, 0, "expected HTTP/2 CONTINUATION");
      }
      if (frame.type() == Http2Frame.SETTINGS) {
        if (frame.streamId() != 0) throw new Http2Exception(1, 0, "SETTINGS uses a stream");
        if ((frame.flags() & ACK) == 0) {
          Settings update = parseSettings(frame.payload());
      ...
      if (frame.type() == Http2Frame.PING) {
        ...
        if ((frame.flags() & ACK) == 0)
          writeFrame(output, new Http2Frame(Http2Frame.PING, ACK, 0, frame.payload()));
        continue;
      }
      ...
      if (frame.type() != Http2Frame.HEADERS && frame.type() != Http2Frame.CONTINUATION
        ...
        throw new Http2Exception(1, 0, "unexpected frame during HTTP/2 WebSocket handshake");
      }
      byte[] fragment =
          frame.type() == Http2Frame.HEADERS ? headerFragment(frame) : frame.payload();
      ...
      if (!continuation) {
        if (responseEndStream) {
          throw new DeploymentException("HTTP/2 WebSocket CONNECT stream ended in the response");
        }
  ...
  private static Settings parseSettings(byte[] payload) throws Http2Exception {
    if (payload.length % 6 != 0) throw new Http2Exception(6, 0, "invalid HTTP/2 SETTINGS length");
    ...
    while (values.hasRemaining()) {
      ...
      if (id == 1) {
        ...
        if (value != 0) throw new Http2Exception(1, 0, "server must disable HTTP/2 push");
      } else if (id == 4) {
        if (value > Integer.MAX_VALUE)
          throw new Http2Exception(3, 0, "invalid HTTP/2 initial window");
        ...
        if (value > 1) throw new Http2Exception(1, 0, "invalid ENABLE_CONNECT_PROTOCOL");
  ...
  private static void applyHandshakeWindowUpdate(Http2Frame frame, Settings settings)
    ...
    if (frame.payload().length != 4) throw new Http2Exception(6, 0, "invalid HTTP/2 WINDOW_UPDATE");
    int value = ByteBuffer.wrap(frame.payload()).getInt() & 0x7fffffff;
    if (value == 0) throw new Http2Exception(1, frame.streamId(), "zero HTTP/2 window increment");
    if (frame.streamId() == 0)
      settings.connectionWindow = addWindow(settings.connectionWindow, value, 0);
    else if (frame.streamId() == STREAM_ID)
      settings.streamWindow = addWindow(settings.streamWindow, value, STREAM_ID);
  }

  private static int addWindow(int current, int increment, int streamId) throws Http2Exception {
    long result = (long) current + increment;
    if (result > Integer.MAX_VALUE) throw new Http2Exception(3, streamId, "HTTP/2 window overflow");
    return (int) result;
  }

  private static Map<String, List<String>> requestHeaders(ClientEndpointConfig config, URI uri) {
  ...
  private static void enforceHeaders(
    ...
    if (config.getPreferredSubprotocols().isEmpty()) {
      ...
      replace(headers, "Sec-WebSocket-Protocol", join(config.getPreferredSubprotocols()));
    }
    if (config.getExtensions().isEmpty()) {
  ...
  private static List<HpackHeader> hpackHeaders(URI uri, Map<String, List<String>> source)
    ...
    result.add(
        new HpackHeader(":scheme", "wss".equalsIgnoreCase(uri.getScheme()) ? "https" : "http"));
    ...
    for (Map.Entry<String, List<String>> entry : source.entrySet()) {
      ...
      if (connectionSpecific(name))
        throw new DeploymentException("connection-specific header is forbidden in HTTP/2: " + name);
      for (String value : entry.getValue()) {
        validHeaderValue(value);
        if ("te".equals(name) && !"trailers".equalsIgnoreCase(value.trim())) {
          throw new DeploymentException("HTTP/2 TE request header may only contain trailers");
        }
  ...
  private static Response response(List<HpackHeader> fields) throws DeploymentException {
    String status = null;
    ...
    for (HpackHeader field : fields) {
      ...
      if (name.startsWith(":")) {
        ...
        headers.computeIfAbsent(name, ignored -> new ArrayList<String>()).add(field.value());
      }
    }
  ...
  private static Negotiated validateResponse(Response response, ClientEndpointConfig config)
      throws DeploymentException {
    if (response.status < 200 || response.status >= 300) {
      throw new DeploymentException("HTTP/2 WebSocket server returned status " + response.status);
    }
    if (response.headers.containsKey("sec-websocket-accept")) {
      throw new DeploymentException("Sec-WebSocket-Accept is forbidden in an RFC 8441 response");
    }
    List<String> protocols = commaTokens(response.headers.get("sec-websocket-protocol"));
    if (protocols.size() > 1)
  ...
  private static void validateCompression(String value, ClientEndpointConfig config)
    ...
    for (int i = 1; i < parts.length; i++) {
      ...
      else throw new DeploymentException("unsupported permessage-deflate response parameter");
    }
    if (!server || !client)
  ...
  private static byte[] headerFragment(Http2Frame frame) throws Http2Exception {
    ...
    if ((frame.flags() & PADDED) != 0) {
      if (payload.length == 0)
        throw new Http2Exception(1, STREAM_ID, "missing HEADERS padding length");
      padding = payload[offset++] & 0xff;
    }
    if ((frame.flags() & PRIORITY) != 0) offset += 5;
    if (offset + padding > payload.length)
      throw new Http2Exception(1, STREAM_ID, "invalid HEADERS padding");
  ...
  private static byte[] dataPayload(Http2Frame frame) throws Http2Exception {
    ...
    if (payload.length == 0) throw new Http2Exception(1, STREAM_ID, "missing DATA padding length");
    int padding = payload[0] & 0xff;
    if (padding >= payload.length) throw new Http2Exception(1, STREAM_ID, "invalid DATA padding");
  ...
  private static Http2Frame readFrame(InputStream input) throws IOException, Http2Exception {
    ...
    if (length > 16_384)
      throw new Http2Exception(6, 0, "peer exceeded the advertised HTTP/2 frame size");
    if ((header[5] & 0x80) != 0)
      throw new Http2Exception(1, 0, "reserved HTTP/2 stream bit is set");
    int streamId =
        ((header[5] & 0x7f) << 24)
            | ((header[6] & 0xff) << 16)
            | ((header[7] & 0xff) << 8)
            | (header[8] & 0xff);
    return new Http2Frame(header[3] & 0xff, header[4] & 0xff, streamId, readFully(input, length));
  }
  ...
  private static void replace(Map<String, List<String>> headers, String name, String value) {
    remove(headers, name);
  ...
  private static final class Settings {
    ...
    private void apply(Settings update) throws Http2Exception {
      ...
      if (update.initialWindowSet) {
        long adjusted = (long) streamWindow + update.peerInitialWindow - peerInitialWindow;
        if (adjusted > Integer.MAX_VALUE || adjusted < Integer.MIN_VALUE) {
          throw new Http2Exception(3, 0, "HTTP/2 stream window overflow");
        }
  ...
  private static final class Handshake extends Negotiated {
    ...
    private Handshake(
        ...
        InputStream input,
        OutputStream output,
        int connectionSendWindow,
        int streamSendWindow,
        int peerInitialWindow,
