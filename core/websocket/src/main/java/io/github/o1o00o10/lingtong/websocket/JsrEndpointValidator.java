...
final class JsrEndpointValidator {
    ...
    private static void validateCodec(Class<?> type, String kind) throws DeploymentException {
        ...
        try {
            ...
            throw new DeploymentException(kind + " requires a public no-argument constructor: "
                    + type.getName(), e);
        }
    }

    private static void validateNonPublicAnnotations(Class<?> endpoint)
            throws DeploymentException {
        for (Class<?> current = endpoint; current != null && current != Object.class;
    ...
    private static void validateLifecycle(
            Method method, Class<? extends Annotation> annotation) throws DeploymentException {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            boolean valid = type == Session.class
    ...
    private static String validateMessage(Method method, EndpointConfig config)
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (pathParam(annotations[i])) {
                ...
                messageType = type;
            } else if ((messageType == boolean.class || messageType == Boolean.class)
                    && !partial) {
                partial = true;
                messageType = type;
            } else {
        ...
        if (nativeType == null) invalid(method, "message type has no Decoder: "
                + messageType.getName());
        if (partial && !(messageType == String.class || messageType == byte[].class
        ...
        for (Type value : type.getGenericInterfaces()) {
            if (value instanceof ParameterizedType) {
                ...
                if (parameterized.getRawType() instanceof Class<?>) {
                    Class<?> target = decoderTarget(
                            (Class<?>) parameterized.getRawType(), decoderInterface);
    ...
    private static void invalid(Method method, String reason) throws DeploymentException {
        throw new DeploymentException("invalid endpoint method "
