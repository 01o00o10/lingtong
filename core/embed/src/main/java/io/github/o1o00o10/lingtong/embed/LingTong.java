  ...
  public static final class Builder {
    ...
    public Builder managementAudit(Path file) {
      return managementAudit(
          file,
          FileManagementAuditSink.DEFAULT_MAX_BYTES,
    ...
    /**
     * Assembles the deployment, Servlet plan, protocol endpoints, and kernel without opening a
     * listener. The caller owns the returned runtime and must close it.
     */
    ...
    public LingTongRuntime build() {
      ...
      try {
        if (webApplication != null) {
          ...
          deployment =
              WebAppDeployment.open(webApplication, maxDeploymentEntries, maxDeploymentBytes);
        ...
        LingTongServerContainer webSocketContainer =
            new LingTongServerContainer(webSockets, maxWebSocketMessageBytes, contextPath);
        if (deployment != null) {
          application.initializer(
              new LingTongWebSocketInitializer(),
              deployment.matchingClasses(
                  ServerEndpoint.class, ServerApplicationConfig.class, Endpoint.class));
        }
        // WebSocket endpoints must be visible while Servlet initializers run.
        application.initializerFirst(
            (classes, context) ->
                context.setAttribute(
                    LingTongServerContainer.SERVLET_CONTEXT_ATTRIBUTE, webSocketContainer));
        application.initializerFirst(
            (classes, context) ->
                context.setAttribute(WebSocketEndpointRegistry.class.getName(), webSockets));
        ServletApplication servletApplication = application.build();
        if (managementToken == null
            && (managementOperatorToken != null
                || managementAuditSink != null
                || managementAuditFile != null
                || managementPort >= 0)) {
          throw new IllegalStateException(
        ...
        DefaultLingTongRuntime runtime =
            new DefaultLingTongRuntime(
                transport,
                workerThreads,
                workerQueueCapacity,
                servletApplication,
