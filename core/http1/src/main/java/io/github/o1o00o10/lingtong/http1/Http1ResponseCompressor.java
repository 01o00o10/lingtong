...
public final class Http1ResponseCompressor {
  ...
  public HttpResponse compress(HttpRequest request, HttpResponse response) {
    ...
    for (Map.Entry<String, List<String>> entry : response.headers().entries()) {
      ...
      for (String value : entry.getValue()) {
        headers.add(
            entry.getKey(), "etag".equalsIgnoreCase(entry.getKey()) ? weakEtag(value) : value);
      }
    }
  ...
  public StreamingPlan prepareStreaming(HttpRequest request, StreamingHttpResponse response) {
    long knownBodyBytes =
        Math.max(response.bodyBytesAtCommit(), declaredContentLength(response.headers()));
    if (!enabled
        || knownBodyBytes < minResponseBytes
    ...
    ResponseBodyMailbox encodedBody =
        new ResponseBodyMailbox(
            response.body().capacityBytes(), response.body().lowWaterBytes(), Long.MAX_VALUE);
    StreamingHttpResponse encodedResponse =
        new StreamingHttpResponse(
            response.status(),
            response.reason(),
  ...
  private boolean compressible(String contentType) {
    ...
    String value =
        (separator < 0 ? contentType : contentType.substring(0, separator))
            .trim()
  ...
  public static final class StreamingPlan {
    ...
    private StreamingPlan(
        ResponseBodyMailbox sourceBody, StreamingHttpResponse response, boolean gzip) {
    ...
    private static StreamingPlan gzip(
        ResponseBodyMailbox sourceBody, StreamingHttpResponse response) {
