...
public final class Http1Encoder {
    ...
    private static final ByteBuffer LAST_CHUNK = ByteBuffer.wrap(
    ...
    public ByteBuffer encode(HttpResponse response, boolean keepAlive, boolean suppressBody) {
        ...
        int contentLength = response.status() == 205 ? 0 : suppressBody || !statusAllowsBody
        ...
        ByteBuffer encoded = ByteBuffer.allocate(
                headerBytes.length + encodedBodyLength + chunkLength + endLength);
    ...
    public ByteBuffer encodeStreamingHead(
        ...
        if (chunked) {
            ...
            head.append("content-length: ")
                    .append(declaredContentLength(response, 0L))
                    .append("\r\n");
        }
