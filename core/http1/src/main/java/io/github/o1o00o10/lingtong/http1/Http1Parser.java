...
public final class Http1Parser {
    ...
    public Http1Parser(int maxHeaderBytes, int maxBodyBytes) {
        this(maxHeaderBytes, maxBodyBytes, false, 0, 0,
                DEFAULT_MAX_HEADER_COUNT, maxHeaderBytes, DEFAULT_MAX_TRAILER_COUNT);
    }
    ...
    public static Http1Parser streaming(
        ...
        return new Http1Parser(
                maxHeaderBytes, maxBodyBytes, true, bodyBufferBytes, bodyLowWaterBytes,
                DEFAULT_MAX_HEADER_COUNT, maxHeaderBytes, DEFAULT_MAX_TRAILER_COUNT);
    }
    ...
    public static Http1Parser streaming(
        ...
        return new Http1Parser(
                maxHeaderBytes, maxBodyBytes, true, bodyBufferBytes, bodyLowWaterBytes,
                maxHeaderCount, maxTrailerBytes, maxTrailerCount);
    }
    ...
    private Http1Parser(
            ...
            int maxTrailerBytes,
            int maxTrailerCount) {
        if (maxHeaderBytes <= 0 || maxBodyBytes < 0 || maxHeaderCount <= 0
                || maxTrailerBytes <= 0 || maxTrailerCount < 0) {
            throw new IllegalArgumentException("invalid parser limits");
        }
        if (streaming && (bodyBufferBytes <= 0
                || bodyLowWaterBytes < 0
    ...
    private boolean readLineByte(byte value) throws HttpParseException {
        ...
        int lineLimit = state == State.TRAILER_LINE
                ? Math.min(headerBuffer.length, maxTrailerBytes) : headerBuffer.length;
    ...
    private void parseRequestLine(String line) throws HttpParseException {
        ...
        if (firstSpace <= 0 || secondSpace <= firstSpace + 1 || line.indexOf(' ', secondSpace + 1) >= 0) {
    ...
    private void validateHost(List<String> values) throws HttpParseException {
        ...
        if (authority.isEmpty() || authority.indexOf(',') >= 0
                ...
                || authority.indexOf('@') >= 0 || authority.indexOf('?') >= 0
                || authority.indexOf('#') >= 0 || containsWhitespace(authority)) {
