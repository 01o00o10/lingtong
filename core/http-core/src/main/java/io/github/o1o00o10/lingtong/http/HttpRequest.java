...
public final class HttpRequest {
    ...
    public HttpRequest(String method, String target, String version, HttpHeaders headers, byte[] body) {
    ...
    public HttpRequest(
        ...
        this(method, target, version, headers, BodyMailbox.completed(body, trailers),
                RequestConnectionInfo.local(headers), null, null);
    }
    ...
    private HttpRequest(
        ...
        if (bodyMailbox == null || connectionInfo == null) {
            throw new IllegalArgumentException("request body mailbox and connection information must not be null");
        }
        this.method = method;
        this.target = target;
    ...
    public static HttpRequest withBodyMailbox(
            ...
            HttpHeaders headers,
            BodyMailbox bodyMailbox) {
        return new HttpRequest(method, target, version, headers, bodyMailbox,
                RequestConnectionInfo.local(headers), null, null);
    }
    ...
    public static HttpRequest extendedConnect(
        ...
        return new HttpRequest("CONNECT", target, "HTTP/2", headers, bodyMailbox,
                RequestConnectionInfo.local(headers), protocol, null);
    }

    /** 保留原正文邮箱，只替换连接元数据。 */
    public HttpRequest withConnectionInfo(RequestConnectionInfo value) {
        return new HttpRequest(method, target, version, headers, bodyMailbox, value,
                extendedConnectProtocol, trace);
    }

    /** 附加请求级诊断时间线，保留原正文邮箱与协议元数据。 */
    public HttpRequest withTrace(RequestTrace value) {
        return new HttpRequest(method, target, version, headers, bodyMailbox, connectionInfo,
                extendedConnectProtocol, value);
    }
