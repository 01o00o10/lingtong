...
final class ServletAttributeEventDispatcher {
    /** 日志记录器。 */
    private static final Logger LOGGER = Logger.getLogger("io.github.o1o00o10.lingtong.servlet.attributes");
    /** empty。 */
    /** 空值标志（EMPTY）。 */
    private static final ServletAttributeEventDispatcher EMPTY = new ServletAttributeEventDispatcher(
            new ServletContextAttributeListener[0],
    ...
    void requestReplaced(ServletContext context, ServletRequest request, String name, Object previous) {
        ServletRequestAttributeEvent event = new ServletRequestAttributeEvent(context, request, name, previous);
        for (ServletRequestAttributeListener listener : requestListeners) {
    ...
    void requestRemoved(ServletContext context, ServletRequest request, String name, Object removed) {
        ServletRequestAttributeEvent event = new ServletRequestAttributeEvent(context, request, name, removed);
        for (ServletRequestAttributeListener listener : requestListeners) {
