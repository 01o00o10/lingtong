...
final class ServletHttpRequest implements HttpServletRequest {
  ...
  ServletHttpRequest(
    ...
    this.characterEncoding =
        contentTypeEncoding == null ? defaultCharacterEncoding : contentTypeEncoding;
    ...
    String cookieSessionId =
        sessionTrackingModes.contains(SessionTrackingMode.COOKIE)
            ? findSessionCookie(request.headers().first("cookie"))
            : null;
  ...
  public Cookie[] getCookies() {
    ...
    for (String pair : header.split(";")) {
      ...
      if (equals > 0) {
        cookies.add(
            new Cookie(pair.substring(0, equals).trim(), pair.substring(equals + 1).trim()));
      }
    }
  ...
  public HttpSession getSession(boolean create) {
    ...
    if (response.isCommitted()) {
      throw new IllegalStateException(
          "cannot create an HTTP session after the response is committed");
    }
  ...
  void saveFormRequest() {
    ...
    current.saveRequest(
        new ServletSecuritySessionState.SavedRequest(
  ...
  private void persistAuthentication(boolean createSession, boolean rotateId) {
    ...
    current.securityState(new ServletSecuritySessionState(securityIdentity, authenticationType));
  ...
  public Collection<Part> getParts() throws IOException, ServletException {
    ...
    if (multipartParts == null) {
      multipartParts =
          MultipartParser.parse(body(), getContentType(), multipartConfig, servletContext);
    }
  ...
  public AsyncContext startAsync(ServletRequest servletRequest, ServletResponse servletResponse) {
    if (!asyncSupported) {
      throw new IllegalStateException(
          "asynchronous processing is disabled by the current Servlet chain");
    }
  ...
  DispatchState beginDispatch(
      ...
      Map<String, String> targetRoleReferences)
  ...
  private Map<String, String[]> parameters() {
    if (parameters == null) {
      ...
      if (contentType != null
        ...
        decodeParameters(new String(body(), StandardCharsets.ISO_8859_1), collected, budget);
  ...
  private void collectMultipartParameters(
      Map<String, List<String>> destination, ServletRequestLimits.ParameterBudget budget) {
    try {
      for (Part part : getParts()) {
        ...
        ByteArrayOutputStream value =
            new ByteArrayOutputStream((int) Math.min(part.getSize(), 4096L));
  ...
  String encodeSessionURL(String url) {
    ...
    if (existing >= 0) {
      ...
      return path.substring(0, existing)
          + ";jsessionid="
          + current.getId()
          + path.substring(end)
  ...
  private List<Locale> acceptedLocales() {
    ...
    Collections.sort(
        preferences,
        new Comparator<LocalePreference>() {
          @Override
