...
final class MultipartParser {
    ...
    static List<MultipartPart> parse(
        ...
        try {
            while (true) {
                ...
                MultipartPart part = new MultipartPart(headers, body, contentStart, size,
                        config.getFileSizeThreshold(), location);
                if (part.getSubmittedFileName() != null
                        && config.getMaxFileSize() >= 0 && size > config.getMaxFileSize()) {
                    part.cleanup();
    ...
    private static String boundary(String contentType) throws ServletException {
        ...
        for (String parameter : contentType.split(";")) {
            ...
            if (value.isEmpty() || value.length() > 70 || value.indexOf('\r') >= 0 || value.indexOf('\n') >= 0) {
                break;
            }
            return value;
        }
    ...
    private static Path uploadLocation(MultipartConfigElement config, ServletContext context) {
        ...
        Path base = temporaryDirectory instanceof File
