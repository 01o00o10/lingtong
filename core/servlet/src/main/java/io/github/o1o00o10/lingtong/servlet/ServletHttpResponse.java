...
final class ServletHttpResponse implements HttpServletResponse {
    ...
    ServletHttpResponse(long maxBodyBytes, String defaultCharacterEncoding) {
        this(maxBodyBytes, 64 * 1024, 32 * 1024, null, defaultCharacterEncoding,
    ...
    ServletHttpResponse(
        ...
        this(maxBodyBytes, responseBodyBufferBytes, responseBodyLowWaterBytes,
                streamingConsumer, null, Collections.<String, String>emptyMap());
    }
    ...
    ServletHttpResponse(
        ...
        this(maxBodyBytes, responseBodyBufferBytes, responseBodyLowWaterBytes,
                streamingConsumer, defaultCharacterEncoding,
                Collections.<String, String>emptyMap());
    }
    ...
    ServletHttpResponse(
        ...
        if (responseBodyBufferBytes <= 0 || responseBodyLowWaterBytes < 0
        ...
        this.defaultCharacterEncoding = defaultCharacterEncoding == null
                ? "ISO-8859-1" : Charset.forName(defaultCharacterEncoding).name();
    ...
    void completeStreaming() {
        ...
        if (completedPublishPending) {
            ...
            streamingConsumer.accept(new StreamingHttpResponse(
                    status,
                    reason,
                    responseHeaders(),
                    responseBody));
    ...
    public void addCookie(Cookie cookie) {
        ...
        StringBuilder encoded = new StringBuilder(cookie.getName()).append('=').append(cookie.getValue());
        ...
        if (cookie instanceof SessionCookieValue
                && ((SessionCookieValue) cookie).sameSite() != null) {
    ...
    private void commitStreaming() throws IOException {
        ...
        committed = true;
        responseBody = new ResponseBodyMailbox(
    ...
    private final class BodyOutputStream extends ServletOutputStream {
        ...
        public synchronized boolean isReady() {
            ...
            return ready && (responseBody == null
                    ? body.size() < maxBodyBytes
                    : responseBody.isWritable());
        }
        ...
        public void setWriteListener(WriteListener writeListener) {
            ...
            try {
                commitStreaming();
                responseBody.onWritable(new Runnable() {
                    @Override
                    public void run() {
                        scheduleWritePossible();
                    }
        ...
        private void requireCapacity(int length) throws IOException {
            if (length < 0 || body.size() > maxBodyBytes - length) {
                ...
                throw new IOException("response body exceeds configured limit of "
                        + maxBodyBytes + " bytes");
            }
        }
