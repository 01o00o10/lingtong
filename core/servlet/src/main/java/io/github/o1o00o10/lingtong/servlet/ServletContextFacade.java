...
final class ServletContextFacade implements InvocationHandler {
    ...
    /** 受限的监听器方法集合。 */
    private static final Set<String> RESTRICTED_LISTENER_METHODS = Collections.unmodifiableSet(
            new HashSet<String>(Arrays.asList(
                    "getEffectiveMajorVersion", "getEffectiveMinorVersion",
                    "setInitParameter", "addServlet", "addJspFile", "createServlet",
                    ...
                    "addListener", "createListener", "getJspConfigDescriptor", "getClassLoader",
                    "declareRoles", "getVirtualServerName", "getSessionTimeout", "setSessionTimeout",
                    "getRequestCharacterEncoding", "setRequestCharacterEncoding",
                    "getResponseCharacterEncoding", "setResponseCharacterEncoding")));
    ...
    private ServletContextFacade(
        ...
        this.applicationClassLoader = contextClassLoader == null
                ...
                : contextClassLoader;
        File tempDir = temporaryDirectory == null
                ? new File(System.getProperty("java.io.tmpdir"))
                : temporaryDirectory.toFile();
        attributes.put(ServletContext.TEMPDIR, tempDir);
        proxy = (ServletContext) Proxy.newProxyInstance(
                ServletContext.class.getClassLoader(),
                new Class<?>[]{ServletContext.class},
                this);
        restrictedProxy = (ServletContext) Proxy.newProxyInstance(
                ServletContext.class.getClassLoader(),
                new Class<?>[]{ServletContext.class},
    ...
    static ServletContextFacade createFacade(RegistrationController registrations, String contextPath) {
    ...
    static ServletContextFacade createFacade(
        ...
        return new ServletContextFacade(
                ServletAttributeEventDispatcher.empty(), registrations, contextPath, null, resources,
    ...
    static ServletContextFacade createFacade(
            ...
            Path temporaryDirectory) {
        return new ServletContextFacade(
                ServletAttributeEventDispatcher.empty(), registrations, contextPath, displayName, resources,
                applicationClassLoader, temporaryDirectory);
    }
    ...
    private ServletRegistration.Dynamic addServlet(Object[] args) throws ServletException {
        ...
        if (component instanceof Class<?>) {
            return registrations.addServlet(registrationName,
                    registrations.createServlet(castClass(component)));
        }
        return registrations.addServlet(registrationName,
    ...
    private FilterRegistration.Dynamic addFilter(Object[] args) throws ServletException {
        ...
        if (component instanceof Class<?>) {
            return registrations.addFilter(registrationName,
                    registrations.createFilter(castClass(component)));
        }
        return registrations.addFilter(registrationName,
                registrations.createFilter(loadClass((String) component, Filter.class)));
    }
    ...
    private void log(Object[] args) {
        ...
        Throwable throwable = args.length > 1 && args[1] instanceof Throwable ? (Throwable) args[1] : null;
        if (throwable == null) {
            LOGGER.info(message);
        } else {
