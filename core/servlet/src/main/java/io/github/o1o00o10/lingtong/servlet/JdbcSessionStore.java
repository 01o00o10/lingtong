    ...
    public SessionRecord load(String id) throws IOException {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            try (ResultSet result = statement.executeQuery()) {
                return result.next() ? new SessionRecord(
                        id, result.getLong(1), result.getLong(2), result.getBytes(3)) : null;
            }
        ...
        try (Connection connection = dataSource.getConnection()) {
            if (expectedVersion < 0L) {
                ...
                try (PreparedStatement statement = connection.prepareStatement(insert)) {
                    ...
                    if (conflict.getSQLState() != null
                            && conflict.getSQLState().startsWith("23")) return -1L;
                    throw conflict;
                }
            }
