...
final class RuntimeMetrics {
    ...
    long uptimeSeconds() { return Math.max(0L, (System.currentTimeMillis() - startedMillis) / 1000L); }
    long requests() { return requests.get(); }
    long failures() { return failures.get(); }
    long responses2xx() { return responses2xx.get(); }
    long responses4xx() { return responses4xx.get(); }
    long responses5xx() { return responses5xx.get(); }
    ...
    long maximumMillis() { return TimeUnit.NANOSECONDS.toMillis(maximumNanos.get()); }
    void managementRequest() { managementRequests.incrementAndGet(); }
    void managementAuthFailure() { managementAuthFailures.incrementAndGet(); }
    void managementAuthorizationFailure() { managementAuthorizationFailures.incrementAndGet(); }
    long managementRequests() { return managementRequests.get(); }
    long managementAuthFailures() { return managementAuthFailures.get(); }
    long managementAuthorizationFailures() { return managementAuthorizationFailures.get(); }
