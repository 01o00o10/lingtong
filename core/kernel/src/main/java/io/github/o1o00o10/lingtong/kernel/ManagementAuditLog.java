...
final class ManagementAuditLog implements AutoCloseable {
    ...
    synchronized void record(
            String remoteAddress, String role, String method,
            String path, int status, String outcome) {
        if (entries.size() == capacity) {
            entries.removeFirst();
            overwritten++;
        }
        ManagementAuditEvent event = new ManagementAuditEvent(
    ...
    synchronized long persistenceFailures() { return persistenceFailures; }
