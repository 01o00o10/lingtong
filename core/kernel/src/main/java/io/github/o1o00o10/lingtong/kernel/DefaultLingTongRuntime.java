...
import static io.github.o1o00o10.lingtong.kernel.ManagementText.decimal;
import static io.github.o1o00o10.lingtong.kernel.ManagementText.jsonEscape;
import static io.github.o1o00o10.lingtong.kernel.ManagementText.jsonNumber;
import static io.github.o1o00o10.lingtong.kernel.ManagementText.metric;
...
/**
 * 连接传输层与 Servlet 应用的运行时。阅读路径：prepare/start -> KernelProcessor -> ServletApplication；停止则按 beginDrain
 * -> stop/close 反向释放。 本层只决定保留路由、WebSocket 握手和指标归属，不解析 HTTP 报文。
 */
public final class DefaultLingTongRuntime implements LingTongRuntime {
  ...
  private static final HttpResponse SERVER_OPTIONS =
      new HttpResponse(
          204, "No Content", HttpHeaders.builder().add("allow", "OPTIONS").build(), new byte[0]);
  ...
  public DefaultLingTongRuntime(
      TransportConfig transportConfig, int workerThreads, int workerQueueCapacity) {
    this(
        transportConfig,
        workerThreads,
        workerQueueCapacity,
        ServletApplication.builder().build(),
        Collections.<String, WebSocketHandler>emptyMap(),
        8 * 1024 * 1024);
  }
  ...
  public DefaultLingTongRuntime(
    ...
    this.auditLog =
        new ManagementAuditLog(256, managementConfig == null ? null : managementConfig.auditSink());
  }
  ...
  public void prepare() throws LingTongException {
    synchronized (lifecycleMonitor) {
      try {
        ...
        LOGGER.log(Level.SEVERE, "runtime preparation failed type=" + e.getClass().getName());
  ...
  public void start() throws LingTongException {
    synchronized (lifecycleMonitor) {
      ...
      NioHttpServer nextServer =
          new NioHttpServer(transportConfig, new KernelProcessor(), nextExecution);
      ...
      if (managementConfig != null && managementConfig.dedicatedListener()) {
        nextManagementExecution = new BoundedExecutionDomain(2, 256, "lingtong-management-worker");
        nextManagementServer =
            new NioHttpServer(
      ...
      try {
              ...
              public void execute(Runnable command) {
                if (!nextExecution.execute(command)) {
                  throw new RejectedExecutionException(
                      "LingTong application execution queue is full");
                }
              }
            });
        ...
        LOGGER.info("runtime started port=" + port() + " managementPort=" + managementPort());
      } catch (Exception | LinkageError e) {
        LOGGER.log(Level.SEVERE, "runtime startup failed type=" + e.getClass().getName());
        running = false;
  ...
  /** 请求路由闸口：管理路由、OPTIONS *、健康检查和 WebSocket 握手优先； 普通业务才进入 ServletApplication。三个入口对应同步、异步、流式传输契约。 */
  private final class KernelProcessor implements StreamingResponseProcessor {
    ...
    public CompletionStage<Void> processStreaming(
      ...
      completion.whenComplete((ignored, failure) -> metrics.end(started, status.get(), failure));
      return completion;
    }
    ...
    private CompletionStage<Void> measuredPublish(
      ...
      result.whenComplete((ignored, failure) -> metrics.end(started, response.status(), failure));
      return result;
    }
    ...
    private CompletionStage<Void> publish(
      ...
      ResponseBodyMailbox body =
          new ResponseBodyMailbox(Math.max(1, bytes.length), 0, Math.max(1, bytes.length));
  ...
  private HttpResponse managementResponse(HttpRequest request, boolean dedicatedListener) {
    ...
    if ("GET".equals(request.method()) && "/__lingtong/admin".equals(path)) {
      if (!dedicatedListener && managementConfig != null && managementConfig.dedicatedListener())
        return NOT_FOUND;
      return managementConfig == null
          ? NOT_FOUND
          : text(200, "OK", "text/html; charset=utf-8", ManagementDashboard.html());
    }
    ...
    if ("/__lingtong/manage/v1/actions/drain".equals(path)) {
      ...
      return audited(
          request,
          path,
          text(202, "Accepted", "application/json; charset=utf-8", "{\"status\":\"draining\"}\n"),
          "drain_started");
    }
  ...
  private ManagementRole managementRole(HttpRequest request) {
    ...
    boolean reader =
        MessageDigest.isEqual(managementConfig.token().getBytes(StandardCharsets.UTF_8), candidate);
    String operatorToken = managementConfig.operatorToken();
    boolean operator =
        operatorToken != null
            && MessageDigest.isEqual(operatorToken.getBytes(StandardCharsets.UTF_8), candidate);
  ...
  private String heartbeatJson() {
    return "{\"nodeId\":\""
        + jsonEscape(managementConfig.nodeId())
        + "\",\"status\":\"UP\",\"uptimeSeconds\":"
  ...
  private String runtimeJson() {
    ...
    return "{\"nodeId\":\""
        ...
        + running
        + ",\"accepting\":"
        + (currentServer != null && currentServer.isAccepting())
        + ",\"applicationPort\":"
        + port()
        + ",\"managementPort\":"
        + managementPort()
        ...
        + metrics.failures()
        + ",\"responses2xx\":"
        + metrics.responses2xx()
        + ",\"responses4xx\":"
        + metrics.responses4xx()
        + ",\"responses5xx\":"
        + metrics.responses5xx()
        + ",\"averageRequestMillis\":"
        + decimal(metrics.averageMillis())
        + ",\"maximumRequestMillis\":"
        + metrics.maximumMillis()
        + ",\"managementRequests\":"
        + metrics.managementRequests()
        + ",\"managementAuthFailures\":"
        + metrics.managementAuthFailures()
        + ",\"managementAuthorizationFailures\":"
        + metrics.managementAuthorizationFailures()
        + ",\"auditEntriesOverwritten\":"
        + auditLog.overwritten()
        + ",\"auditPersistenceFailures\":"
        ...
        + ",\"localSessions\":"
        + application.activeSessionCount()
        + ",\"storedSessions\":"
        + application.storedSessionCount()
        + ",\"expiredSessions\":"
        + application.expiredSessionCount()
        + ",\"sessionScavengerFailures\":"
  ...
  private String clusterJson() {
    StringBuilder json =
        new StringBuilder("{\"nodeId\":\"")
            .append(jsonEscape(managementConfig.nodeId()))
            .append("\",\"peers\":[");
    ...
    for (int i = 0; i < peers.size(); i++) {
      ...
      json.append("{\"endpoint\":\"")
          .append(jsonEscape(peer.endpoint))
          .append("\",\"nodeId\":")
          .append(peer.nodeId == null ? "null" : "\"" + jsonEscape(peer.nodeId) + "\"")
          .append(",\"available\":")
          .append(peer.available)
          .append(",\"latencyMillis\":")
          .append(peer.latencyMillis)
          .append(",\"lastAttemptMillis\":")
          .append(peer.lastAttemptMillis)
          .append(",\"lastSuccessMillis\":")
          .append(peer.lastSuccessMillis)
          .append(",\"probes\":")
          .append(peer.probes)
          .append(",\"failures\":")
          .append(peer.failures)
          .append(",\"consecutiveFailures\":")
          .append(peer.consecutiveFailures)
          .append(",\"failure\":")
  ...
  private String diagnosticsJson() {
    ...
    StringBuilder json =
        new StringBuilder("{\"classLoading\":{\"loaded\":")
            .append(classes.getLoadedClassCount())
            .append(",\"totalLoaded\":")
            .append(classes.getTotalLoadedClassCount())
            .append(",\"unloaded\":")
            .append(classes.getUnloadedClassCount())
            .append("},\"operatingSystem\":{\"name\":\"")
            .append(jsonEscape(operatingSystem.getName()))
            .append("\",\"arch\":\"")
            .append(jsonEscape(operatingSystem.getArch()))
            .append("\",\"availableProcessors\":")
            .append(operatingSystem.getAvailableProcessors())
            .append(",\"systemLoadAverage\":")
            .append(jsonNumber(operatingSystem.getSystemLoadAverage()))
    ...
    for (int i = 0; i < collectors.size(); i++) {
      ...
      if (i > 0) json.append(',');
      json.append("{\"name\":\"")
          .append(jsonEscape(collector.getName()))
          .append("\",\"collectionCount\":")
          .append(collector.getCollectionCount())
          .append(",\"collectionTimeMillis\":")
          .append(collector.getCollectionTime())
          .append('}');
    }
    ...
    for (int i = 0; i < pools.size(); i++) {
      ...
      if (i > 0) json.append(',');
      json.append("{\"name\":\"")
          .append(jsonEscape(pool.getName()))
          .append("\",\"type\":\"")
          .append(pool.getType())
          .append("\",\"usedBytes\":")
          .append(usage == null ? -1L : usage.getUsed())
          .append(",\"committedBytes\":")
          .append(usage == null ? -1L : usage.getCommitted())
          .append(",\"maxBytes\":")
          .append(usage == null ? -1L : usage.getMax())
          .append('}');
    }
    return json.append("]}\n").toString();
  }
  ...
  private String threadsJson() {
    ...
    for (ThreadInfo info : information) {
      ...
      json.append("{\"id\":")
          ...
          .append(",\"name\":\"")
          .append(jsonEscape(info.getThreadName()))
          .append("\",\"state\":\"")
  ...
  private String auditJson() {
    ...
    StringBuilder json =
        new StringBuilder("{\"overwritten\":")
            .append(auditLog.overwritten())
            .append(",\"persistenceFailures\":")
            .append(auditLog.persistenceFailures())
            .append(",\"entries\":[");
    for (int i = 0; i < entries.size(); i++) {
      ManagementAuditEvent entry = entries.get(i);
      if (i > 0) json.append(',');
      json.append("{\"timestampMillis\":")
          .append(entry.timestampMillis())
          .append(",\"remoteAddress\":\"")
          .append(jsonEscape(entry.remoteAddress()))
          .append("\",\"role\":\"")
          .append(jsonEscape(entry.role()))
          .append("\",\"method\":\"")
          .append(jsonEscape(entry.method()))
          .append("\",\"path\":\"")
          .append(jsonEscape(entry.path()))
          .append("\",\"status\":")
          .append(entry.status())
          .append(",\"outcome\":\"")
          .append(jsonEscape(entry.outcome()))
          .append("\"}");
    }
    return json.append("]}\n").toString();
  }
  ...
  private String prometheus() {
    ...
    metric(
        output,
        "lingtong_management_auth_failures_total",
        "counter",
        metrics.managementAuthFailures());
    metric(
        output,
        "lingtong_management_authorization_failures_total",
        "counter",
        metrics.managementAuthorizationFailures());
    metric(
        output, "lingtong_management_audit_overwritten_total", "counter", auditLog.overwritten());
    metric(
        output,
        "lingtong_management_audit_persistence_failures_total",
        "counter",
        auditLog.persistenceFailures());
    metric(
        output,
        "lingtong_active_connections",
        "gauge",
        currentServer == null ? 0 : currentServer.activeConnections());
    metric(
        output,
        "lingtong_connections_accepted_total",
        "counter",
        currentServer == null ? 0 : currentServer.acceptedConnections());
    metric(
        output,
        "lingtong_connections_rejected_total",
        "counter",
        currentServer == null ? 0 : currentServer.rejectedConnections());
    metric(
        output,
        "lingtong_worker_active",
        "gauge",
        currentExecution == null ? 0 : currentExecution.activeCount());
    metric(
        output,
        "lingtong_worker_queued",
        "gauge",
        currentExecution == null ? 0 : currentExecution.queuedCount());
    ...
    metric(
        output,
        "lingtong_session_scavenger_failures_total",
        "counter",
        application.sessionScavengerFailureCount());
    metric(
        output,
        "lingtong_jvm_heap_used_bytes",
        "gauge",
        runtime.totalMemory() - runtime.freeMemory());
    metric(output, "lingtong_jvm_heap_max_bytes", "gauge", runtime.maxMemory());
    metric(
        output,
        "lingtong_jvm_threads",
        "gauge",
        ManagementFactory.getThreadMXBean().getThreadCount());
    ...
    try {
      String node =
          managementConfig == null
              ? "runtime-" + System.identityHashCode(this)
              : managementConfig.nodeId();
      ObjectName name =
          new ObjectName("io.github.o1o00o10.lingtong:type=Runtime,node=" + ObjectName.quote(node));
  ...
  private final class RuntimeMBean implements LingTongRuntimeMXBean {
    ...
    private RuntimeMBean(String nodeId) {
      this.nodeId = nodeId;
    }
    ...
    public boolean isAcceptingRequests() {
      return application.isAcceptingRequests();
    }
    ...
    public int getLocalSessions() {
      return application.activeSessionCount();
    }

    @Override
    public long getStoredSessions() {
      return application.storedSessionCount();
    }

    @Override
    public long getExpiredSessions() {
      return application.expiredSessionCount();
    }

    @Override
    public long getSessionScavengerFailures() {
      return application.sessionScavengerFailureCount();
    }

    @Override
    public long getManagementAuthFailures() {
      return metrics.managementAuthFailures();
    }

    @Override
    public long getManagementAuthorizationFailures() {
      return metrics.managementAuthorizationFailures();
    }

    @Override
    public long getManagementAuditOverwritten() {
      return auditLog.overwritten();
    }

    @Override
    public long getManagementAuditPersistenceFailures() {
