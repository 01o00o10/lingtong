...
public final class ManagementConfig {
    ...
    public ManagementConfig(
            String token, String nodeId, List<String> peers,
            long heartbeatMillis, int failureThreshold) {
        this(token, nodeId, peers, heartbeatMillis, failureThreshold, "127.0.0.1", -1);
    }

    public ManagementConfig(
            String token, String nodeId, List<String> peers, long heartbeatMillis,
            int failureThreshold, String bindAddress, int port) {
        this(token, nodeId, peers, heartbeatMillis, failureThreshold,
                bindAddress, port, null, null);
    }

    public ManagementConfig(
            String token, String nodeId, List<String> peers, long heartbeatMillis,
            int failureThreshold, String bindAddress, int port,
            String operatorToken, ManagementAuditSink auditSink) {
        if (token == null || token.length() < 16 || nodeId == null
                || !nodeId.matches("[A-Za-z0-9._-]{1,128}")
                || peers == null || heartbeatMillis < 250L
                || failureThreshold <= 0 || failureThreshold > 100
                || bindAddress == null || bindAddress.trim().isEmpty()
                || port < -1 || port > 65535
    ...
    public long heartbeatMillis() { return heartbeatMillis; }
    public int failureThreshold() { return failureThreshold; }
    public String bindAddress() { return bindAddress; }
    public int port() { return port; }
    public boolean dedicatedListener() { return port >= 0; }
    public String operatorToken() { return operatorToken; }
    public ManagementAuditSink auditSink() { return auditSink; }
}
