...
public final class HpackCodec {
  ...
  public HpackCodec(int maxHeaderListBytes) {
    if (maxHeaderListBytes <= 0)
      throw new IllegalArgumentException("header list limit must be positive");
  ...
  public List<HpackHeader> decode(byte[] block) throws Http2Exception {
    ...
    while (cursor.hasRemaining()) {
      ...
      listBytes +=
          32
              + header.name().getBytes(StandardCharsets.UTF_8).length
  ...
  private HpackHeader indexed(int index) throws Http2Exception {
    ...
    if (dynamicIndex < 0 || dynamicIndex >= dynamic.size())
      throw compression("invalid table index");
  ...
  private static int entrySize(HpackHeader h) {
    return 32
        + h.name().getBytes(StandardCharsets.UTF_8).length
