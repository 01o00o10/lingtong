...
public final class Http2ServerConnection {
  ...
  public void respond(int streamId, HttpResponse response, Consumer<ByteBuffer> output)
    ...
    boolean suppressBody =
        "HEAD".equals(stream.requestMethod)
            || response.status() == 204
            || response.status() == 205
            || response.status() == 304
    ...
    stream.responseTrailers =
        suppressBody || !hasTrailers(response.trailers()) ? null : response.trailers();
    if (!hasLength)
      headers.add(
          new HpackHeader(
              "content-length",
              Integer.toString(suppressBody ? responseBody.length : body.length)));
    byte[] block = encoder.encode(headers);
    writeHeaderBlock(streamId, block, body.length == 0 && stream.responseTrailers == null, output);
    if (body.length == 0) {
  ...
  private void handle(Http2Frame frame, Listener listener, Consumer<ByteBuffer> output)
    ...
    switch (frame.type()) {
      case Http2Frame.SETTINGS:
        settings(frame, output);
        break;
      case Http2Frame.PING:
        ping(frame, output);
        break;
      case Http2Frame.HEADERS:
        headers(frame, listener, output);
        break;
      case Http2Frame.CONTINUATION:
        continuation(frame, listener);
        break;
      case Http2Frame.DATA:
        data(frame, listener, output);
        break;
      case Http2Frame.WINDOW_UPDATE:
        windowUpdate(frame, output);
        break;
      ...
      case Http2Frame.PUSH_PROMISE:
        throw connection(1, "client sent PUSH_PROMISE");
      default:
        break;
    }
  }
  ...
  private void dispatch(Stream stream, Listener listener) throws Http2Exception {
    ...
    for (HpackHeader field : stream.headers) {
      ...
      if (!name.equals(name.toLowerCase(Locale.ROOT)))
        throw stream(1, stream.id, "uppercase field name");
      if (name.startsWith(":")) {
    ...
    if (extendedConnect) {
      listener.onRequest(
          stream.id,
          HttpRequest.extendedConnect(
              protocol,
              path,
              headers.build(),
              BodyMailbox.completed(new byte[0], HttpHeaders.builder().build())));
    } else {
      listener.onRequest(
          stream.id,
          new HttpRequest(
  ...
  private void windowUpdate(Http2Frame frame, Consumer<ByteBuffer> output) throws Http2Exception {
    ...
    if (frame.streamId() == 0) {
      if ((long) connectionSendWindow + increment > Integer.MAX_VALUE)
        throw connection(3, "connection window overflow");
      connectionSendWindow += increment;
      for (Stream stream : new ArrayList<Stream>(streams.values())) flushData(stream, output);
    } else {
      Stream stream = streams.get(frame.streamId());
      if (stream == null) return;
      if ((long) stream.sendWindow + increment > Integer.MAX_VALUE)
        throw stream(3, stream.id, "stream window overflow");
  ...
  private void reset(Http2Frame frame, Listener listener) throws Http2Exception {
    if (frame.streamId() == 0 || frame.payload().length != 4)
      throw connection(6, "invalid RST_STREAM");
  ...
  private void priority(Http2Frame frame) throws Http2Exception {
    if (frame.streamId() == 0 || frame.payload().length != 5)
      throw connection(6, "invalid PRIORITY");
  }
  ...
  private void flushData(Stream stream, Consumer<ByteBuffer> output) {
    ...
    if (stream.responseMailbox != null
    ...
    while (connectionSendWindow > 0 && stream.sendWindow > 0) {
      if (stream.responseChunk == null) {
        if (stream.responseBody != null) {
          ...
          stream.responseChunk =
              stream.activeTunnelWrite == null ? null : stream.activeTunnelWrite.bytes;
          stream.responseOffset = 0;
          if (stream.responseChunk == null) {
      ...
      int length =
          Math.min(
              remaining,
              Math.min(peerMaxFrameSize, Math.min(connectionSendWindow, stream.sendWindow)));
      ...
      boolean end =
          chunkEnd
              ...
              && !stream.tunnel;
      ByteBuffer encoded = frame(Http2Frame.DATA, end ? END_STREAM : 0, stream.id, payload);
      CompletableFuture<Void> completion =
          stream.tunnel && chunkEnd && stream.activeTunnelWrite != null
              ? stream.activeTunnelWrite.completion
              : null;
  ...
  private static void emit(
      Consumer<ByteBuffer> output, ByteBuffer frame, CompletableFuture<Void> completion) {
  ...
  private void writeHeaderBlock(
      int streamId, byte[] block, boolean endStream, Consumer<ByteBuffer> output) {
    int offset = 0;
    boolean first = true;
    do {
      ...
      output.accept(
          frame(first ? Http2Frame.HEADERS : Http2Frame.CONTINUATION, flags, streamId, fragment));
      first = false;
  ...
  private int activeStreams() {
    int active = 0;
    for (Stream stream : streams.values())
      if (!stream.reset && !(stream.localEnd && stream.remoteEnd)) active++;
    return active;
  }
  ...
  private static String unique(String current, String value, int streamId, String name)
      throws Http2Exception {
  ...
  private static Http2Exception connection(int error, String message) {
    return new Http2Exception(error, 0, message);
  }

  private static Http2Exception stream(int error, int streamId, String message) {
    return new Http2Exception(error, streamId, message);
  }
  ...
  private static final class Stream {
    ...
    private Stream(int id, int sendWindow) {
      this.id = id;
