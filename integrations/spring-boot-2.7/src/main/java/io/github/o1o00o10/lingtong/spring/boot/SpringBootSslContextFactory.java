...
final class SpringBootSslContextFactory {
  ...
  static SSLContext create(Ssl ssl, SslStoreProvider stores) throws Exception {
    ...
    try {
      ...
      keyManagers = selectAlias(keyManagerFactory.getKeyManagers(), keyStore, ssl.getKeyAlias());
    } finally {
  ...
  private static KeyManager[] selectAlias(KeyManager[] managers, KeyStore keyStore, String alias)
      throws Exception {
    ...
    if (!wrapped) {
      throw new IllegalArgumentException("server.ssl.key-alias requires an X509 key manager");
    }
    return result;
  }
