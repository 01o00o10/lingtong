...
public class LingTongServletWebServerFactoryAutoConfiguration {
  ...
  public LingTongServletWebServerFactory lingTongServletWebServerFactory(
    ...
    factory.setRequestBodyBufferBytes(
        bytes(properties.getRequestBodyBufferSize(), "request body buffer size"));
    factory.setRequestBodyLowWaterBytes(
        bytes(properties.getRequestBodyLowWaterSize(), "request body low water size"));
    factory.setResponseBodyBufferBytes(
        bytes(properties.getResponseBodyBufferSize(), "response body buffer size"));
    factory.setResponseBodyLowWaterBytes(
        bytes(properties.getResponseBodyLowWaterSize(), "response body low water size"));
    ...
    factory.setManagementAuditMaxBytes(
        bytes(properties.getManagementAuditMaxSize(), "management audit max size"));
  ...
  public WebServerFactoryCustomizer<LingTongServletWebServerFactory>
    ...
    return factory -> {
      if (properties.getMaxHttpHeaderSize() != null) {
        factory.setMaxHeaderBytes(
            bytes(properties.getMaxHttpHeaderSize(), "maximum HTTP header size"));
      }
    };
  }
