...
final class LingTongConfigurationCompatibilityCustomizer
    ...
    private static final Set<String> TOMCAT_ALIASES = Collections.unmodifiableSet(
    ...
    public void customize(LingTongServletWebServerFactory factory) {
        ...
        if (!unsupported.isEmpty()) {
            throw new WebServerException(
                    "LingTong cannot apply container-specific properties " + unsupported
    ...
    private void mapTomcatAliases(LingTongServletWebServerFactory factory) {
        if (configured("server.tomcat.connection-timeout")
            ...
            factory.setRequestHeaderTimeout(bind(
                    "server.tomcat.connection-timeout", Duration.class));
        }
        ...
        if (configured("server.tomcat.max-keep-alive-requests")
            ...
            factory.setMaxRequestsPerConnection(bind(
                    "server.tomcat.max-keep-alive-requests", Integer.class));
        }
        if (configured("server.tomcat.max-http-form-post-size")
                && !configured("server.lingtong.max-parameter-size")) {
            factory.setMaxParameterBytes(bytes(bind(
                    "server.tomcat.max-http-form-post-size", DataSize.class),
                    "server.tomcat.max-http-form-post-size"));
        }
        if (configured("server.tomcat.threads.max")
                && !configured("server.lingtong.worker-threads")) {
    ...
    private <T> T bind(String name, Class<T> type) {
        return binder.bind(name, Bindable.of(type)).orElseThrow(() ->
